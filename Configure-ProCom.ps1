...
function Write-Log {
  ...
  Add-Content -Path $Script:Logfile -Value "$date : $Message"
  if ($Script:VerboseLogging) {
    Write-Host "$date - $Message" -ForegroundColor Cyan
  }
  # Error message on try-catch: $_.Exception.Message
}
...
function ChoicePicker_User {
  ...
  # Create a local user with with password
  if ($userPassword -ne "") {    
...
function ChoicePicker_Office {
  ...
  try {
    ...
    Write-Log "Error creating Office configuration XML: $_.Exception.Message"
  }
  ...
  try {
    ...
    Write-Log "Error locating Office Deployment Tool: $_.Exception.Message"
  }
  ...
  try {
    Remove-Item $setupPath -Force -Recurse -ErrorAction SilentlyContinue
    Remove-Item $xmlPath -Force -Recurse -ErrorAction SilentlyContinue
  }
  catch {
    write-Log "Error cleaning up Office installation files: $_.Exception.Message"
  }
...
function functionPicker_Rename_User {
  ...
  try {
    ...
    Write-Log "Error renaming user or profile folder: $_.Exception.Message"
  }
